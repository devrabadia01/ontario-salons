...
def load_data():
    ...
    for el in elements:
        ...
        # build address similar to your Node version
...
# ----------------- FILTERS (similar logic to your Node app) -----------------
...
with left:
    search = st.text_input("Search by name / city / 'Niagara':", "")
with right:
    ...
    if type_choice != "All":
        if type_choice.startswith("hairdresser"):
            ...
            if "barber" not in name:
                return False
        elif "salon" in type_choice and "saloon" not in type_choice:
...
# Center roughly on Ontario
m = folium.Map(location=[44, -79.5], zoom_start=5, tiles="OpenStreetMap")
...
for _, row in filtered.iterrows():
    ...
    links = []
    if website:
        links.append(f'<a href="{website}" target="_blank" rel="noopener">Website</a>')
...
with st.expander("Show data table"):
    st.dataframe(filtered[["name", "shop", "address", "city", "phone", "website"]])
